...
3. **Data attributes are added**: Adds `data-has-responsive-prop="true"` to enable CSS rules
4. **CSS media queries apply**: Uses the CSS custom properties with fallback chains
...
```tsx
<Grid columns={{ base: 1, sm: 2, lg: 4 }} gap={{ base: 'xs', md: 'lg' }} />
```
...
- Grid/Flex properties (columns, gap, direction)
